...
def parse_iqtree_Q(iqtree_path: str) -> np.ndarray:
    """
    Extract the 4x4 GTR rate matrix Q (ACGT row/column order)
...
class SequenceMutator:
    ...
    def augment_to_target(
        ...
        For each synthetic record:
          ...
          3. A target year is sampled uniformly in [parent_year+1, MAX_YEAR].
        ...
        for k in range(1, n_to_gen + 1):
            ...
            tgt_year = self._py_rng.randint(src_year+1, MAX_YEAR)
