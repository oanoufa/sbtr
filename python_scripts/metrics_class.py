...
class HIVSubtypingMetrics:
    ...
    def __init__(self, num_subtypes: int, split: str, output_path: str = None,
        ...
        self.tp_counts = torch.zeros(num_subtypes)
        self.fn_counts = torch.zeros(num_subtypes)
    ...
    def reset(self):
        ...
        self.tp_counts.zero_()
        self.fn_counts.zero_()
    ...
    def update(self, preds, targets, loss_mask, loss, input_ids=None, loss_unreduced=None):
        ...
        with torch.no_grad():
            fp = (preds_binary == 1) & (targets_flat == 0)
            self.fp_confusion += (fp.float().T @ targets_flat.float()).cpu()
            tp = (preds_binary == 1) & (targets_flat == 1)
            fn = (preds_binary == 0) & (targets_flat == 1)
            self.tp_counts += tp.float().sum(dim=0).cpu()
            self.fn_counts += fn.float().sum(dim=0).cpu()
    ...
    def compute_detailed(self):
        ...
        result["tp_counts"]      = self.tp_counts.clone()
        result["fp_confusion"]   = self.fp_confusion.clone()
        result["fn_counts"]      = self.fn_counts.clone()
        return result
    ...
    def print_detailed(self, top_n=50):
